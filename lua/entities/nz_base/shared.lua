...
function ENT:CheckRangeToEnemy()
	...
	if self:GetEnemy() then
		...
		if nb_npc:GetInt() == 1 then
			...
			if ( (enemy:IsNPC() or enemy:GetClass() == "chance_plastic") and enemy:Health() > 0 ) or ( enemy:IsPlayer() and enemy:Alive() ) then
...
function ENT:OnInjured( dmginfo )
	...
	if ( dmginfo:IsDamageType(8) ) then
		self:Ignite(10,30)
...
function ENT:SetEnemy( ent )
	...
	if (ent != nil and !ent:IsVehicle()) then
		if !ent:IsValid() then return end
...
function ENT:CheckTargetMethod()
	...
	if nb_targetmethod:GetInt() == 1 then
		...
		if self:IsLineOfSightClear( self ) then 
...
function ENT:SearchForEnemy( ents )
	...
	for k,v in pairs( ents ) do
		...
		if nb_npc:GetInt() == 1 then
			...
			--[[elseif (v:IsPlayer() and v:GetVehicle() != nil) then
				print(v:GetVehicle())
				self:SetEnemy( v:GetVehicle() )--]]
			else
				if v:IsNPC() and v != self and !string.find(v:GetClass(), "npc_nextbot_*") and !string.find(v:GetClass(), "npc_bullseye") and !string.find(v:GetClass(), "npc_grenade_frag") and !string.find(v:GetClass(), "animprop_generic") then
					self:SetEnemy( v )
					return true
				...
				if(v:GetClass() == "chance_plastic") then
					self:SetEnemy(v)
					return true
				end
			end
...
function ENT:HaveEnemy()
	...
	if ( enemy and IsValid( enemy ) and enemy:GetMoveType() != MOVETYPE_NOCLIP ) then
		if ( enemy:IsPlayer() and !enemy:Alive() ) then
